...
while cap.isOpened():
    ...
    for i, rect in enumerate(rects):
        ...
        d1, d2, d3, d4 = np.linalg.norm(landmarks[37] - landmarks[41]), np.linalg.norm(landmarks[38] - landmarks[40]), np.linalg.norm(landmarks[43] - landmarks[47]), np.linalg.norm(landmarks[44] - landmarks[46])
        d_mean = (d1 + d2 + d3 + d4) / 4
        d5, d6 = np.linalg.norm(landmarks[36] - landmarks[39]), np.linalg.norm(landmarks[42] - landmarks[45])
        d_reference = (d5 + d6) / 2
